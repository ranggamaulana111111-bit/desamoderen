<x-admin-layout title="Surat Masuk" maxWidth="max-w-[1600px]">

    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Surat Masuk</h1>
            <p class="text-gray-500 mt-1 text-sm">Kelola data surat masuk desa.</p>
        </div>
        ...
        <div class="stat-micro bg-white border border-gray-200/80 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Total</p>
                </div>
            </div>
        </div>
        <div class="stat-micro bg-white border border-gray-200/80 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
                <div>
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['hari_ini'] }}</p>
                    <p class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Hari Ini</p>
                </div>
            </div>
        </div>
        <div class="stat-micro bg-white border border-gray-200/80 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['minggu_ini'] }}</p>
                    <p class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Minggu Ini</p>
                </div>
            </div>
        </div>
        <div class="stat-micro bg-white border border-gray-200/80 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Bulan Ini</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="px-5 py-4">
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nomor surat, pengirim, atau perihal..."
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse ($surat as $item)
                        <tr class="group hover:bg-gray-50/50 transition-colors">
                            <td class="px-5 py-4">
                                <span class="font-mono text-xs font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg">{{ $item->nomor_agenda }}</span>
                            </td>
                            <td class="px-5 py-4">
                                <p class="font-semibold text-gray-900 group-hover:text-emerald-700 transition max-w-[180px] truncate">{{ $item->nomor_surat }}</p>
                            </td>
                            <td class="px-5 py-4 text-gray-600 max-w-[150px] truncate">{{ $item->pengirim }}</td>
                            <td class="px-5 py-4 text-gray-600 max-w-[200px] truncate">{{ $item->perihal }}</td>
                            <td class="px-5 py-4 text-center">
                                @php
                                    $sifatBadge = match($item->sifat_surat) {
                                ...
                                @endphp
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold {{ $sifatBadge }}">
                                    {{ $item->sifat_surat }}
                                </span>
                            </td>
                            <td class="px-5 py-4 text-center">
                                @php
                                    $statusBadge = match($item->status) {
                                        'diterima' => 'bg-blue-50 text-blue-700 border border-blue-100',
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    <a href="{{ route('admin.surat-masuk.edit', $item) }}"
                                    ...
                                    <form action="{{ route('admin.surat-masuk.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus surat masuk ini?')">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-16">
                                <div class="empty-state">
                                    ...
                                    <p class="text-sm text-gray-400 font-medium">Belum ada surat masuk</p>
                                    <a href="{{ route('admin.surat-masuk.create') }}" class="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 hover:text-emerald-700 transition">
