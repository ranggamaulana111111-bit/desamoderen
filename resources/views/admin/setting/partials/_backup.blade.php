@include('admin.setting.partials._active_note', [
    'message' => 'Backup aktif: snapshot database (dan file storage) disimpan ke storage/app/backups, dapat diunduh kapan saja. Jalankan penjadwal Laravel agar auto-backup berjalan sesuai jadwal.',
])
<form x-show="activeTab === 'backup'" x-cloak
    ...
    <div class="setting-card bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6 space-y-5">
            <div class="rounded-xl border border-sky-100 bg-sky-50/60 p-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                ...
                <form action="{{ route('admin.setting.backupRun') }}" method="POST" @submit="saving = true">
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-medium text-sm px-5 py-2.5 rounded-xl transition shadow-sm" :disabled="saving">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-blue-50/50 transition opacity-70">
    ...
    <div class="divide-y divide-gray-100">
        ...
        <div class="px-6 py-4 flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                <a href="{{ route('admin.setting.backupDownload', $backup['filename']) }}" class="inline-flex items-center gap-1.5 text-xs font-semibold text-sky-700 bg-sky-50 hover:bg-sky-100 px-3 py-2 rounded-lg transition">
