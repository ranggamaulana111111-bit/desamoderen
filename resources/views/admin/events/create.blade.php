<x-admin-layout title="Buat Event Baru" maxWidth="max-w-[1200px]">
    ...
    <div x-data="{
        ...
        tanggal: '{{ old('tanggal') }}',
        waktuMulai: '{{ old('waktu_mulai', '09:00') }}',
        waktuSelesai: '{{ old('waktu_selesai') }}',
        tempat: '{{ old('tempat') }}',
        rtTarget: '{{ old('rt_target') }}',
        rwTarget: '{{ old('rw_target') }}',
        get statusLabel() {
                        ...
                        <div class="widget-card-body space-y-5">
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" x-model="deskripsi" rows="3"
                                    ...
                                    class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition">{{ old('deskripsi') }}</textarea>
                                @error('deskripsi')
                                    <p class="text-red-500 text-xs mt-1.5 flex items-center gap-1">
                                        <svg class="w-3.5 h-3.5 shrink-0" fill="currentColor" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8"/><text x="10" y="13.5" text-anchor="middle" fill="white" font-size="11" font-weight="bold">!</text></svg>
                                        {{ $message }}
