<x-lembaga-layout title="{{ $event->judul }}">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl font-bold text-slate-900 mt-2">{{ $event->judul }}</h1>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('lembaga.events.edit', $event) }}" class="btn-ghost inline-flex items-center gap-2">Ubah</a>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bento-card p-6">
                <form action="{{ route('lembaga.events.destroy', $event) }}" method="POST" onsubmit="return confirm('Hapus event ini? Tindakan tidak dapat dibatalkan.')">
                    @csrf
                    @method('DELETE')
