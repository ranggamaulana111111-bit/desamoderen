...
<body>
    ...
    <div class="isi-spesifik">
        Bahwa orang tersebut di atas adalah benar warga Desa {{ config('village.nama_desa', 'Desa') }}, Kecamatan {{ config('village.nama_kecamatan', 'Kecamatan') }}, {{ config('village.nama_kabupaten', 'Kabupaten') }} dan sedang dalam proses pembuatan Kartu Tanda Penduduk (KTP) sehingga memerlukan surat keterangan pengganti KTP sementara. Surat keterangan ini berlaku selama <strong>30 (tiga puluh) hari</strong> sejak tanggal diterbitkan.
    </div>
    @if (!empty($alasan_ktp))
    <div class="isi-spesifik">
        Alasan pengajuan: {{ $alasan_ktp }}.
    </div>
    @endif
