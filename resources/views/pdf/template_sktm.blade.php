...
<body>
    ...
    <div class="isi-spesifik">
        Bahwa orang tersebut di atas adalah benar warga Desa {{ config('village.nama_desa', 'Desa') }}, Kecamatan {{ config('village.nama_kecamatan', 'Kecamatan') }}, {{ config('village.nama_kabupaten', 'Kabupaten') }} dan menurut pengakuan yang bersangkutan serta diketahui oleh perangkat desa setempat, termasuk golongan <strong>keluarga tidak mampu</strong> secara ekonomi dengan penghasilan Rp{{ number_format($penghasilan, 0, ',', '.') }},- per bulan.
    </div>
    <div class="isi-spesifik">
        Surat keterangan ini dibuat untuk keperluan <strong>{{ $keperluan ?? $alasan_sktm }}</strong>.
    </div>
