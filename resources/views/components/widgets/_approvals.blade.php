<div class="grid grid-cols-1 xl:grid-cols-12 gap-5">
    <div class="xl:col-span-7 widget-card overflow-hidden">
        ...
        <div class="divide-y divide-gray-50" x-data="{ rejectModal: false, rejectId: null }">
            @if (!empty($approvals) && $approvals['total'] > 0)
                ...
                <div class="flex items-center gap-3.5 px-5 py-3.5 hover:bg-gray-50/50 transition group">
                    ...
                    <div class="flex items-center gap-1.5 shrink-0">
                        <form action="{{ route('admin.pengajuan.approve', $item['id']) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="w-8 h-8 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 hover:text-green-700 hover:shadow-sm transition-all flex items-center justify-center" title="Setujui">
                        ...
                        <button type="button" @click="rejectId = {{ $item['id'] }}; rejectModal = true" class="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 hover:shadow-sm transition-all flex items-center justify-center" title="Tolak">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                        <a href="{{ $item['url'] }}" class="w-8 h-8 rounded-lg bg-gray-50 text-gray-400 hover:bg-gray-100 hover:text-gray-600 hover:shadow-sm transition-all flex items-center justify-center" title="Detail">
                ...
                <div class="px-5 py-12 text-center">
                    ...
                    <p class="text-sm text-gray-500 font-medium mt-3">Semua pengajuan sudah diproses</p>
                    <p class="text-xs text-gray-400 mt-0.5">Tidak ada approval yang menunggu</p>
                </div>
                @endforelse
            @else
                <div class="px-5 py-12 text-center">
    ...
    <div class="xl:col-span-5 widget-card overflow-hidden">
        <div class="widget-card-header">
            <div class="flex items-center gap-2.5">
        ...
        <div class="max-h-[400px] overflow-y-auto">
            ...
            @empty
            <div class="px-5 py-12 text-center">
                <div class="empty-state-icon bg-gray-50 mx-auto border border-gray-100">
                    <svg class="w-5 h-5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
                <p class="text-sm text-gray-400 font-medium mt-3">Belum ada aktivitas</p>
            </div>
            @endforelse
        </div>
    </div>
